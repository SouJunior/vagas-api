...
export class CreateJobService {
  constructor(private jobRepository: JobRepository, private mailService: MailService) { }

  async execute(data: CreateJobDto, company: CompaniesEntity) {
    const { salaryMin, salaryMax, federalUnit, modality, indefinideContract, contractType,
      affirmative, affirmativeType } = data;

    if (salaryMin > salaryMax) {
      throw new BadRequestException('Informe um valor final maior do que o valor inicial descrito');
    }

    if (modality != JobsModalityEnum.REMOTE && !federalUnit) {
      throw new BadRequestException('Para vagas diferentes de remoto, obrigatorio inserir a unidade federativa')
    }
    if (!indefinideContract && !contractType) {
      throw new BadRequestException('Quando o contrato não for indeterminado, informe o tempo de contrato(contractType)');
    }
